...
if [ "xcompleted" != "x`syscfg get psm_migration`" ];then
	if [ "$MODEL_NUM" = "CGM4140COM" ] || [ "$MODEL_NUM" = "CGM4981COM" ] || [ "$MODEL_NUM" == "CGM601TCOM" ] ||  [ "$MODEL_NUM" == "SG417DB-CT" ] || [ "$MODEL_NUM" = "CGM4331COM" ];then
		rm -rf "$MIGRATION_FILE"
		psmcli set dmsb.l2net.1.Members.SW ""
		psmcli set dmsb.l2net.9.Members.SW ""
		if [ "$MODEL_NUM" = "CGM4331COM" ] || [ "$MODEL_NUM" = "CGM4981COM" ] || [ "$MODEL_NUM" == "CGM601TCOM" ] || [ "$MODEL_NUM" == "SG417DB-CT" ];then
...
if [ "$migrationCompleteFlag" -eq 0 ];then
	if [ "$MODEL_NUM" = "CGM4140COM" ] || [ "$MODEL_NUM" = "TG3482G" ] || [ "$MODEL_NUM" = "CGM4981COM" ] || [ "$MODEL_NUM" == "CGM601TCOM" ] || [ "$MODEL_NUM" == "SG417DB-CT" ] || [ "$MODEL_NUM" = "CGM4331COM" ] || [ "$MODEL_NUM" = "TG4482A" ];then
		for i in 1 2
		do
