...
static int add_dhcpv4_opt_to_list (dhcp_opt_list ** opt_list, int opt, char * opt_val)
{
    ...
    if ( opt_val != NULL )
    {
       unsigned int len = strlen(opt_val);
       new_dhcp_opt->dhcp_opt_val = malloc(len+1);
       if(new_dhcp_opt->dhcp_opt_val == NULL)
	       return RETURN_ERR;
       strncpy(new_dhcp_opt->dhcp_opt_val,opt_val,len+1);
    }
    else
    {
...
static int get_dhcpv4_opt_list (dhcp_opt_list ** req_opt_list, dhcp_opt_list ** send_opt_list)
{
    ...
    if (syscfg_get(NULL, "eth_wan_enabled", wanoe_enable, sizeof(wanoe_enable)) == 0)
    {
        if (strcmp(wanoe_enable, "true") == 0)
        {
            ...
            add_dhcpv4_opt_to_list(send_opt_list, DHCPV4_OPT_60, "eRouter1.0 dslforum.org");
            //add_dhcpv4_opt_to_list(req_opt_list, DHCPV4_OPT_125, NULL); Need to be revisited
            //add_dhcpv4_opt_to_list(req_opt_list, DHCPV4_OPT_43, NULL); As per requirement option 43 should not be in the requested list.
            prepare_dhcp43_optvalue(options, VENDOR_OPTIONS_LENGTH, wanoe_enable);
